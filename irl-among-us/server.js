...
io.on('connection', (socket) => {
    ...
    socket.on('actionKill', (data) => {
        ...
        if (killer && target && target.status === 'alive') {
    ...
    socket.on('actionShield', (data) => {
        ...
        if (doctor && target && doctor.role === 'doctor' && doctor.status === 'alive' && target.status === 'alive') {
            const currentShieldTarget = room.players.find(p => p.shieldedBy === doctor.id);
            if (currentShieldTarget && currentShieldTarget.id !== target.id) {
                currentShieldTarget.shieldedBy = null;
            }
            if (target.shieldUsed && target.shieldedBy !== doctor.id) {
                return;
            }
            if (target.shieldedBy === doctor.id) {
                io.to(data.room).emit('updateGame', room.players);
                return;
            }
            target.shieldedBy = doctor.id;
            io.to(data.room).emit('updateGame', room.players);
        }
    });
    ...
    socket.on('actionJail', (data) => {
        ...
        if (jailor && target && jailor.role === 'jailor' && jailor.status === 'alive' && target.status === 'alive' && jailor.jailorUses > 0) {
            const currentJailedTarget = room.players.find(p => p.jailedBy === jailor.id);
            if (currentJailedTarget && currentJailedTarget.id !== target.id) {
                currentJailedTarget.jailedBy = null;
            }
...
function evaluateVotes(roomCode) {
    ...
    const voteTally = Object.entries(tally)
        .filter(([id]) => id !== 'skip')
        .map(([id, count]) => {
            const player = room.players.find(p => p.id === id);
            return { id, username: player ? player.username : 'Unknown', count };
        })
        .sort((a, b) => b.count - a.count);

    io.to(roomCode).emit('meetingVoteTally', { tally: voteTally });
